...
#weather effect and remove the seasonal effects
...
#make individual plots for each region with a quadratic fit (curve_fit)
for i in range(len(list_of_regions)):
	...
	#quadratic fit centered at MMT (estimated with LOWESS over temperature)
	x = pd.to_numeric(df[x_data], errors='coerce').values
	y = pd.to_numeric(df[y_data], errors='coerce').values
	...
	if x.size >= 10:
		...
		stdx = np.nanstd(xc) if np.nanstd(xc) > 0 else 1.0
		w = 1.0 / (1.0 + (xc / stdx)**2)
		sigma = 1.0 / np.sqrt(w)

		#use Lp loss with 2 < p < 4 (middle ground that punishes large errors more than LSQ)
		try:
			p = 3.0
			eps = 1e-12
			...
			def res(params):
				...
				if sigma is not None:
					r = r / sigma
				#least_squares minimizes sum(residual^2); return |r|^(p/2) * sign(r)
				return np.sign(r) * np.power(np.abs(r) + eps, p / 2.0)

			#initial guess
			if xc.size >= 3:
				p0 = np.polyfit(xc, y, 2)
			else:
				p0 = [0.0, 0.0, np.nanmedian(y)]

			ls = least_squares(res, x0=p0, max_nfev=20000)
			popt = ls.x
		except Exception:
			popt, pcov = curve_fit(quad, xc, y, maxfev=20000)
