...
FEATS = ["tempmax","tempmin","snowdepth","humidity"]#,"windspeed","precip","snowdepth","humidity"] #features to train on
...
def make_loaders(df: pd.DataFrame):
    ...
    df["excess_deaths"] = pd.to_numeric(df["excess_deaths"], errors="coerce")

    df = df.dropna(subset=FEATS + ["excess_deaths"]).reset_index(drop=True)
    ...
    mu_y = tr["excess_deaths"].mean()
    sd_y = tr["excess_deaths"].std()
    sd_y = 1.0 if (sd_y == 0 or np.isnan(sd_y)) else sd_y

    ytr = ((tr["excess_deaths"] - mu_y) / sd_y).to_numpy(float).reshape(-1,1)
    yva = ((va["excess_deaths"] - mu_y) / sd_y).to_numpy(float).reshape(-1,1) if len(va) else np.empty((0,1))
