...
#To plot at the end, we need tempmax and deaths present
df_plot = df.dropna(subset=["tempmax", "deaths"]).copy()
...
for reg in regions:
    ...
    region_full = df[df["region"] == reg][FEAT_COLS + ["deaths"]].reset_index(drop=True)

    #rows for plotting (require tempmax + target present)
    region_plot = df_plot[df_plot["region"] == reg][["tempmax", "deaths"]].reset_index(drop=True)
    ...
    #two-panel plots (tempmax vs deaths + bottom bar counts)
