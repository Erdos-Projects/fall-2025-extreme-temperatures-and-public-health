...
for reg in regions:
    ...
    if "pred_minus_actual" not in df_pred.columns:
        df_pred["pred_minus_actual"] = df_pred["predicted_deaths"] - df_pred["actual_deaths"]

    y_train_true = pd.to_numeric(df_train["deaths"], errors="coerce").to_numpy(float)
    y_test_true  = pd.to_numeric(df_pred["actual_deaths"], errors="coerce").to_numpy(float)
    y_test_hat   = pd.to_numeric(df_pred["predicted_deaths"], errors="coerce").to_numpy(float)
