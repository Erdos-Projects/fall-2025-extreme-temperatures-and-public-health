# 9_simple_polyfit_mse.py  (updated to also report MAE)
import os
import numpy as np
import pandas as pd
...
def load_xy(csv_path: str):
    ...
    y = pd.to_numeric(df["deaths"], errors="coerce").to_numpy(float)
...
for reg in regions:
    ...
    #compute metrics
    mse_train = mse(ytr, ytr_hat)
    mse_test  = mse(yte, yte_hat)
...
total_row = {
    ...
    "mse_train": mse(Ytr_all, Ytrhat_all),
    "mse_test": mse(Yte_all, Ytehat_all),
    "mae_train": mae(Ytr_all, Ytrhat_all),
    "mae_test": mae(Yte_all, Ytehat_all)}

summary = pd.DataFrame(rows + [total_row])

for i in ["mse_train","mse_test","mae_train","mae_test"]:
