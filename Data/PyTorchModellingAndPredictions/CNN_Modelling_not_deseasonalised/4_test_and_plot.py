...
for reg in regions:
	...
	y = pd.to_numeric(df.get("deaths", pd.Series(index=df.index, dtype=float)), errors="coerce")
	...
	#Save CSV
	out_df = pd.DataFrame({"tempmax": t.to_numpy(float),"predicted_deaths": yhat,"actual_deaths": y.to_numpy(float),})
	...
	ax_scatter.scatter(out_df["tempmax"], out_df["predicted_deaths"],s=4, alpha=1., label="Predicted (TEST)", zorder = 1)
	ax_scatter.scatter(out_df["tempmax"], out_df["actual_deaths"],s=4, alpha=1., label="Actual (TEST)", zorder = 0)
	ax_scatter.set_ylabel("Deaths", labelpad=-1)
	ax_scatter.set_title(f"{reg} — Tempmax vs Deaths")
	ax_scatter.legend()
	...
	ax_scatter2.scatter(out_df["tempmax"], out_df["predicted_deaths"],s=4, alpha=1.)
	ax_scatter2.set_ylabel("Predicted deaths", labelpad=-1)
